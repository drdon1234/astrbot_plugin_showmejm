...
async def before_download(event: AstrMessageEvent, options: JmOptions, manga_id):
    try:
        ...
        try:
            ...
            await event.send(event.plain_result(f"下载时出现问题:{str(e)}"))
        ...
        if len(pdf_files) > 0:
            ...
            if event.is_private_chat():
                await send_files_in_order(options, event, pdf_files, manga_id, single_file_flag, is_group=False)
            else:
                await send_files_in_order(options, event, pdf_files, manga_id, single_file_flag, is_group=True)
        else:
            print("没有找到下载的pdf文件")
            await event.send(event.plain_result("没有找到下载的pdf文件"))

    except Exception as e:
        await event.send(event.plain_result(f"代码运行时出现问题:{str(e)}"))
