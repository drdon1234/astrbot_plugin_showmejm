...
def download_and_get_pdf(options, arg):
    """下载并生成PDF"""
    import jmcomic
...
def generate_pdf(options, input_folder, pdf_path, pdf_name):
    # 收集所有图片路径并排序
    image_paths = []
    ...
    for idx, chunk in enumerate(chunks, 1):
        final_pdf = os.path.abspath(os.path.join(pdf_path, f"{pdf_name}-{idx}.pdf"))
