...
class MessageAdapter:
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.platform_type = self.config.get("platform", {}).get("type", "napcat")
        self.http_host = self.config.get("platform", {}).get("http_host", "127.0.0.1")
        self.http_port = self.config.get("platform", {}).get("http_port", 8080)
        self.api_token = self.config.get("platform", {}).get("api_token", "")
